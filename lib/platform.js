  ...
  async initialiseDevice(device) {
    try {
      ...
      if (platformConsts.devices.switchMultiPower.includes(uiid) && device.params.workMode === 2) {
        ...
        // Expose a standalone 'Auto AC' virtual switch as its own bridged accessory.
        // Same eweDeviceId so the redundant-accessory prune keeps it; no device updates route here.
        const uuidAC = this.api.hap.uuid.generate(`${device.deviceid}SWAC`)
        let acAcc = devicesInHB.get(uuidAC)
        if (!acAcc) {
          acAcc = new this.api.platformAccessory('Auto AC', uuidAC)
          acAcc
            .getService(this.api.hap.Service.AccessoryInformation)
            .setCharacteristic(this.api.hap.Characteristic.Name, 'Auto AC')
            .setCharacteristic(this.api.hap.Characteristic.ConfiguredName, 'Auto AC')
            .setCharacteristic(this.api.hap.Characteristic.SerialNumber, `${device.deviceid}SWAC`)
            .setCharacteristic(this.api.hap.Characteristic.Manufacturer, device.brandName)
          acAcc.context.eweDeviceId = device.deviceid
          this.api.registerPlatformAccessories(plugin.name, plugin.alias, [acAcc])
          this.configureAccessory(acAcc)
        }
        const acSwitch = acAcc.getService(this.api.hap.Service.Switch)
          || acAcc.addService(this.api.hap.Service.Switch, 'Auto AC')
        acSwitch.getCharacteristic(this.api.hap.Characteristic.On).onSet((value) => {
          this.log('[%s] Auto AC [%s].', device.name, value ? 'on' : 'off')
        })
        /** */
