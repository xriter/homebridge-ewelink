...
export default class {
    constructor(platform, accessory, acAccessory) {
        ...
        const deviceConf =
            platform.deviceConf[accessory.context.eweDeviceId] || {};
        this.tempOffset =
            deviceConf.offset || platformConsts.defaultValues.offset;
        ...
        this.cacheTemp = this.tempService.getCharacteristic(
        ...
        this.dewService =
            this.accessory.getServiceById(
                this.hapServ.TemperatureSensor,
                "dewpoint",
            ) ||
        ...
        this.cacheHumi = this.humiService.getCharacteristic(
        ...
        this.co2Service =
            this.accessory.getService(this.hapServ.CarbonDioxideSensor) ||
            this.accessory.addService(this.hapServ.CarbonDioxideSensor);
        this.cacheCO2 = this.co2Service.getCharacteristic(
            this.hapChar.CarbonDioxideLevel,
        ).value;
        ...
        if (!platform.config.disableNoResponse) {
            this.tempService
                ...
                .onGet(() => {
                    ...
                    return this.tempService.getCharacteristic(
                        this.hapChar.CurrentTemperature,
                    ).value;
                });
            this.humiService
                ...
                .onGet(() => {
                    ...
                    return this.humiService.getCharacteristic(
                        this.hapChar.CurrentRelativeHumidity,
                    ).value;
                });
            this.airQualityService
                .getCharacteristic(this.hapChar.AirQuality)
                .onGet(() => {
                    if (!this.isOnline) {
                        throw new this.hapErr(-70402);
                    }
                    return this.airQualityService.getCharacteristic(
                        this.hapChar.AirQuality,
                    ).value;
                });
            this.co2Service
                .getCharacteristic(this.hapChar.CarbonDioxideDetected)
                .onGet(() => {
                    if (!this.isOnline) {
                        throw new this.hapErr(-70402);
                    }
                    return this.co2Service.getCharacteristic(
                        this.hapChar.CarbonDioxideDetected,
                    ).value;
                });
            this.dewService
                .getCharacteristic(this.hapChar.CurrentTemperature)
                .onGet(() => {
                    ...
                    return this.dewService.getCharacteristic(
                        this.hapChar.CurrentTemperature,
                    ).value;
                });
        }
        ...
        if (acAccessory) {
            this.acToggle =
                acAccessory.getService(this.hapServ.Switch) ||
                acAccessory.addService(
                    this.hapServ.Switch,
                    "Auto AC",
                    "autoac",
                );
        ...
        this.accessory.eveService = new platform.eveService(
            "custom",
            this.accessory,
            {
                log: () => {},
    ...
    async externalUpdate(params) {
        try {
            ...
            if (this.cacheHumi > 0) {
                ...
                const b = 243.04;
                const g =
                    Math.log(this.cacheHumi / 100) +
                    (a * this.cacheTemp) / (b + this.cacheTemp);
                const newDew = Math.round(((b * g) / (a - g)) * 10) / 10;
                if (newDew !== this.cacheDew) {
                    this.cacheDew = newDew;
                    this.dewService.updateCharacteristic(
                        this.hapChar.CurrentTemperature,
                        this.cacheDew,
                    );
                }
    ...
    // From 21-22°C, only run on low when the dew point is above 11.5°C; map 22°C+ by dew point.
    ...
    updateACLevel(autoOverride) {
        ...
        if (
            ...
            level = "low";
        } else if (auto && this.cacheTemp >= 22) {
            // Hysteresis: drop a level at the nominal boundary, but require +0.2°C above it to climb back up,
            // so a dew point sitting on a boundary doesn't flap the AC on and off.
            const up = { off: 11.7, low: 13.7, med: 14.2 }; // dew needed to climb to the next level
            const down = { low: 11.5, med: 13.5, high: 14.0 }; // dew at/below which we drop a level
            const dew = this.cacheDew;
            let i = Math.max(0, order.indexOf(this.cacheACLevel)); // start from the current level
            while (i < 3 && dew >= up[order[i]]) {
                i += 1;
            }
            while (i > 0 && dew <= down[order[i]]) {
                i -= 1;
            }
            level = order[i];
        }
        ...
        for (const key of Object.keys(this.acLevels)) {
            this.acLevels[key].updateCharacteristic(
                this.hapChar.OccupancyDetected,
                key === level ? 1 : 0,
            );
        }
        if (this.enableLogging) {
    ...
    currentState() {
        ...
        toReturn.temperature = {
            current: this.tempService.getCharacteristic(
                this.hapChar.CurrentTemperature,
            ).value,
        };
        toReturn.humidity = {
            current: this.humiService.getCharacteristic(
                this.hapChar.CurrentRelativeHumidity,
            ).value,
        };
        toReturn.airQuality = {
            current: this.airQualityService.getCharacteristic(
                this.hapChar.AirQuality,
            ).value,
        };
        toReturn.carbonDioxide = {
            level: this.co2Service.getCharacteristic(
                this.hapChar.CarbonDioxideLevel,
            ).value,
            detected: this.co2Service.getCharacteristic(
                this.hapChar.CarbonDioxideDetected,
            ).value,
        };
        return toReturn;
